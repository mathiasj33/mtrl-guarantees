...
def clopper_pearson_bounds(successes, trials, beta):
    """Compute Clopper-Pearson lower bounds."""
    # Lower bound using beta distribution
    lower = stats.beta.ppf(beta, successes, trials - successes + 1)
...
def main(cfg: DictConfig):
    ...
    logger.info(
        f"Computing guarantees with beta={cfg.bounds.beta}, delta={cfg.bounds.delta}"
    )
    # Compute Clopper-Pearson bounds from the available data
    lower_bounds = clopper_pearson_bounds(
        df["num_successes"].values, df["num_episodes"].values, cfg.bounds.beta
    )
