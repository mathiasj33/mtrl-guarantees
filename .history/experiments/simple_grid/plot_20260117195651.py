...
def main(cfg: DictConfig):
    ...
    # Apply very light smoothing to red curve with minimal window to maintain monotonicity
    y_red = guarantees["probs"].values
    window = 3
    y_smoothed_red = np.convolve(y_red, np.ones(window)/window, mode='same')
    # Enforce monotonicity by keeping original where smoothed version breaks it
    for i in range(1, len(y_smoothed_red)):
        if y_smoothed_red[i] < y_smoothed_red[i-1]:
            y_smoothed_red[i] = y_smoothed_red[i-1]
