...
def main(cfg: DictConfig):
    ...
    df_computed = pd.DataFrame({
        ...
        "y": guarantees["probs"]
    }).drop_duplicates(subset=['x']).sort_values('x')
    
    spl_computed = make_interp_spline(df_computed["x"], df_computed["y"], k=3)
    x_smooth_computed = np.linspace(df_computed["x"].min(), df_computed["x"].max(), 300)
    ...
    df_actual = pd.DataFrame({
        "x": actual_guarantees["guarantees"],
        "y": actual_guarantees["probs"]
    }).drop_duplicates(subset=['x']).sort_values('x')
    
    spl_actual = make_interp_spline(df_actual["x"], df_actual["y"], k=3)
    x_smooth_actual = np.linspace(df_actual["x"].min(), df_actual["x"].max(), 300)
