...
def setup_latex_fonts() -> None:
    # If LaTeX is installed, this gives the full LaTeX look.
    # If not, set text.usetex=False and it will still look CM-like.
    plt.rcParams.update(
        {
            "text.usetex": True,
            "font.family": "serif",
...
def apply_anthropic_style(ax: plt.Axes) -> None:
    ...
    # very subtle dashed major grid only
    ax.grid(
        True,
        which="major",
        linestyle=(0, (3, 3)),
    ...
    # show all spines, thicker, clean
    for side in ["top", "right", "left", "bottom"]:
        ax.spines[side].set_visible(True)
        ax.spines[side].set_color(ANTHROPIC["ink"])
        ax.spines[side].set_linewidth(1.8)

    # ticks: a bit thicker, labels slightly smaller than axis labels
    ax.tick_params(
        axis="both",
        which="major",
        labelsize=12,
...
def main(cfg: DictConfig):
    ...
    # Smaller figure; labels kept big so they feel larger relative to the plot
    ...
    ax1.plot(
        ...
        linewidth=3.6,  # slightly thicker so light color still reads well
        linestyle="--",
        dash_capstyle="round",
        label=r"\textbf{Actual Guarantees}",
    )

    ax1.set_title(r"\textbf{Performance Guarantees}", fontsize=13, color=ANTHROPIC["ink"], pad=8)
    ax1.set_xlabel(r"\textbf{Performance Guarantee }$t$", fontsize=14, color=ANTHROPIC["ink"], labelpad=6)
    ax1.set_ylabel(r"\textbf{Satisfaction Probability }$(1-\delta)$", fontsize=14, color=ANTHROPIC["ink"], labelpad=6)

    # tighter, cleaner legend
    ax1.legend(
        frameon=False,
        loc="lower left",
        fontsize=11,
    ...
    # Extra breathing room while keeping plots compact
