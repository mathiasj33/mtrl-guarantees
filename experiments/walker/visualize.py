...
def get_checkpoint_path(cfg: DictConfig) -> Path:
    ...
    # Default: look in runs/walker/walker_ckp
    default_path = Path("runs/walker/walker_ckp")
...
@hydra.main(
    version_base="1.1", config_path="../../conf", config_name="visualize_walker"
)
def main(cfg: DictConfig):
    """Main visualization function."""
    logger.info("Starting Walker visualization")
    ...
    task = WalkerTaskParams(
        ...
        damping_scale=jnp.array(cfg.task.damping_scale),
    )
    logger.info(
        f"Task parameters: mass={cfg.task.mass_scale}, size={cfg.task.size_scale}, damping={cfg.task.damping_scale}"
    )
